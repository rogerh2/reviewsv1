	...
	class ThemeIsle_SDK_Logger {
		...
		function get_html( $key ) {
			...
			return '<div >'
				   ...
				   . '<div class="actions">'
				   . get_submit_button(
					   __( $button_submit, 'hestia' ), 'primary ' . $this->product->get_key() . '-ti-logger', $this->product->get_key() . 'ti-logger-yes', false, array(
						   'data-ti-log-enable' => 1,
					   )
				   )
				   . get_submit_button(
					   __( $button_cancel, 'hestia' ), 'secondary ' . $this->product->get_key() . '-ti-logger', $this->product->get_key() . 'ti-logger-no', false, array(
