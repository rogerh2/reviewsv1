    ...
    class twitter_bootstrap_nav_walker extends Walker_Nav_Menu {
        ...
        function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0) {
            ...
            if (strcasecmp($item->title, 'divider')) {
                ...
                $item_output .= ($args->has_children) ? ' <i class="fa fa-plus"></i></a>' : '</a>';
...
function get_video_image($url, $post_ID, $img_quality) {
    ...
    if (!empty($url)) {
        ...
        if (!empty($pos_vimeo)) {
            ...
            ?>
            <img src="<?php echo $data[0]->thumbnail_large; ?>" class="vimeo-thumbnail" title="<?php echo get_the_title($post_ID) ?>" alt="<?php echo get_the_title($post_ID) ?>" />
        <?php
        }
        if (empty($pos_vimeo) && empty($pos_youtube)) {
...
function dimox_breadcrumbs() {
    ...
    $delimiter = ' <span class="divider"><i class="fa fa-circle"></i></span> ';
...
function saveGallery4Id($post_ID) {
    ...
    if($template_name == "templates/template_gallery_4_columns.php") {
